...
Gvar	:VAR				{GInstall($1->varname,currtype,1,paramlist_head);
						reset_paramlist();
					}	
	|VAR '['NUM']' 			{GInstall($1->varname,currtype,$3->val,paramlist_head);
						reset_paramlist();}
	|VAR '('ParamList')' 		{GInstall($1->varname,currtype,-1,paramlist_head); 
						reset_paramlist();
					}
	|VAR '(' ')'			{GInstall($1->varname,currtype,-1,paramlist_head);
						reset_paramlist();}
	;
...
Fdef	:NameParam '{'LdeclBlock Body'}'
	...
	;
NameParam:RetTypeName FuncName '('ParamList')'{name_equivalence($1,$2->varname,paramlist_head); 
						reset_paramlist();}
	 |RetTypeName FuncName '('')' 	  {name_equivalence($1,$2->varname,NULL);
						reset_paramlist(); }
	 ;
...
Slist	:Slist Stmt	{$$=create_tree(0,TLookup("null"),NULL,conn_node,$1,NULL,$2); }	
	|Stmt		{$$=$1; }	
	;
...
Stmt	:InputStmt		{$$=$1;}
	...
	|FuncStmt		{$$=$1;}
	|FreeStmt		{$$=$1;}
	|CONTINUE';'		{$$=create_tree(0,TLookup("null"),NULL,continue_node,NULL,NULL,NULL); }
	|BREAK';'		{$$=create_tree(0,TLookup("null"),NULL,break_node,NULL,NULL,NULL); }
	;

InputStmt:READ '('VAR')'';'	{$$=create_tree(0,TLookup("null"),NULL,read_node,$3,NULL,NULL); }
	 |READ '('Sarray')'';'	{$$=create_tree(0,TLookup("null"),NULL,read_node,$3,NULL,NULL); }
	 |READ '('Field')'';'	{$$=create_tree(0,TLookup("null"),NULL,read_node,$3,NULL,NULL); }
	;
OutputStmt:WRITE '('expr')' ';'	{$$=create_tree(0,TLookup("null"),NULL,write_node,$3,NULL,NULL); }
	  ;
AssgStmt:VAR '=' expr ';'	{
					typecheck($1);
...
/*	|Sarray'='dexpr';'	{$$=create_tree(0,NULL,"=",assg_node,$1,NULL,$3); }
	...
	;
dexpr	:ALLOC '(' ')'		{$$=create_tree(0,TLookup("null"),NULL,alloc_node,NULL,NULL,NULL);}
	|INITIALIZE '(' ')'	{$$=create_tree(0,TLookup("int"),NULL,initialize_node,NULL,NULL,NULL);}
	;
...
expr	:expr PLUS expr		{$$=create_tree(0,TLookup("int"),"+",aop_node,$1,NULL,$3); }	
	|expr MINUS expr	{$$=create_tree(0,TLookup("int"),"-",aop_node,$1,NULL,$3); }
	|expr MUL expr		{$$=create_tree(0,TLookup("int"),"*",aop_node,$1,NULL,$3); }
	|expr DIV expr		{$$=create_tree(0,TLookup("int"),"/",aop_node,$1,NULL,$3); }
	|expr MOD expr		{$$=create_tree(0,TLookup("int"),"%",aop_node,$1,NULL,$3); }
		
	|expr LE expr		{$$=create_tree(0,TLookup("bool"),"<=",lop_node,$1,NULL,$3); }
	|expr GE expr		{$$=create_tree(0,TLookup("bool"),">=",lop_node,$1,NULL,$3); }
	|expr LT expr		{$$=create_tree(0,TLookup("bool"),"<",lop_node,$1,NULL,$3); }
	|expr GT expr		{$$=create_tree(0,TLookup("bool"),">",lop_node,$1,NULL,$3); }
	|expr EE expr		{$$=create_tree(0,TLookup("bool"),"==",lop_node,$1,NULL,$3); }
	|expr NE expr		{$$=create_tree(0,TLookup("bool"),"!=",lop_node,$1,NULL,$3); }

	|expr ANDTK expr        {$$=create_tree(0,TLookup("bool"),"&&",lop_node,$1,NULL,$3); }
	|expr ORTK expr		{$$=create_tree(0,TLookup("bool"),"||",lop_node,$1,NULL,$3); }
	|    NOTTK expr		{$$=create_tree(0,TLookup("bool"),"!",lop_node,$2,NULL,NULL); }
...
Field	:VAR '.' VAR	{
	...
	;
Fcall	:VAR '(' ')'	     {$$=create_tree(0,TLookup("null"),$1->varname,func_node,NULL,NULL,NULL); }
  	|VAR '(' ArgList ')' {$$=create_tree(0,TLookup("null"),$1->varname,func_node,$3,NULL,NULL); }
	;
